...
function distancia(grafo, nodo) {
  ...
  for (var i = 0; i < grafo.length-1; i++) {
    grafo[i].dis = Math.sqrt(Math.pow(grafo[i].cord[0] - grafo[nodo].cord[0], 2) + Math.pow(grafo[i].cord[1] - grafo[nodo].cord[1], 2))
      ...
      for (var i = 0; i < nodos.length; i++) {
        if (grafo[nodos[i][0]].vis == 'white') {
          ...
          switch (tipo) {
            ...
            case 5:
              grafo[nodos[i][0]].c = grafo[u[0]].c + nodos[i][1]
              costo = grafo[nodos[i][0]].c + grafo[nodos[i][0]].dis
